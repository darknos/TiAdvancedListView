...
@interface TiAdvancedListViewListSectionProxy ()
@property (nonatomic, readonly) id<TiUIListViewDelegate> dispatcher;
@end

@implementation TiAdvancedListViewListSectionProxy {
...
- (void)updateItemAt:(id)args
{
	...
	[self.dispatcher dispatchUpdateAction:^(UITableView *tableView) {
		...
		if (!forceReload) {
			TiAdvancedListViewListItem *cell = (TiAdvancedListViewListItem *)[tableView cellForRowAtIndexPath:[indexPaths objectAtIndex:0]];
